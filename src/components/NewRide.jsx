...
// NumberInput is defined OUTSIDE AddRide so React doesn't recreate it on every
// render — previously defined inside caused inputs to remount on every keystroke,
// losing focus and causing keyboard flicker on mobile.
function NumberInput({ label, value, onChange, placeholder, prefix, suffix, color, disabled, inputStyle, labelStyle, theme }) {
  return (
    <div style={{ flex: 1 }}>
      <label style={{ 
        ...labelStyle, 
...
export default function AddRide({ 
  ...
  const handleSave = useCallback(() => {
    ...
    // onSave already navigates to dashboard inside App.jsx's addRide —
    // calling onBack() here too caused a double-navigation that blanked the screen.
    setTimeout(() => {
      onSave({
        platform,
        fare: fareN,
      ...
      setSaving(false);
      // REMOVED: onBack() — was causing blank screen on 2nd+ ride entry
    }, 600);
